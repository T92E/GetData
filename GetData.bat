@echo off
::(T92E) 2020/06/07-2020/06/07
title GetData && color 0a && cls
set LA=%cd:~0,1%
set L=%cd:~0,2%
set CuPath=%~dp0
::---------------------------------------------------
rem Change the location of the results at will (The results will be overwrite each time, keep copy to save them)
set ResultsPath=%CuPath%\Results
::---------------------------------------------------
...
set CuFo=systeminfo
echo %CuFo%...
systeminfo >%ResultsPath%\%CuFo%.txt
cls

set CuFo=ipconfig
echo %CuFo%...
ipconfig >%ResultsPath%\%CuFo%.txt
cls

set CuFo=ipconfigAll
echo %CuFo%...
ipconfig /all >%ResultsPath%\%CuFo%.txt
cls

set CuFo=hostname
echo %CuFo%...
hostname >%ResultsPath%\%CuFo%.txt
cls

set CuFo=netAccounts
echo %CuFo%...
net accounts >%ResultsPath%\%CuFo%.txt
cls
...
title %CuFo%
cls
if not exist %ResultsPath%\%CuFo% md %ResultsPath%\%CuFo%
::-------------------------------------------------------------------------------------
if %TreeMode% == 1 (
    goto SkipTrees
) else if %TreeMode% == 2 (
    ...
    for %%p in (C D E F) do (
        if exist %%p:\ (
            ...
            echo Tree: %%p...
            %%p:
            tree /f /a > %ResultsPath%\%CuFo%\%%p.txt
            cls
            )
        )
	)
...
if %WmicMode% == 1 (
    ...
    echo %%g...
    wmic %%g list brief > %ResultsPath%\%CuFo%\%%g.txt
    cls
    )
goto SkipWmics
cls
...
if %WmicMode% == 3 (
    ...
    echo %%g...
    wmic %%g list > %ResultsPath%\%CuFo%\%%g.txt
    cls
    )
goto SkipWmics
cls
